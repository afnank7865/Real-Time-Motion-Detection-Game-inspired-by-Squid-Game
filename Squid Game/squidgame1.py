...
import os
import pygame   # ✅ new import for audio

# ✅ Initialize pygame mixer once
pygame.mixer.init()

cap = cv2.VideoCapture(1)
url="http://192.168.1.15/1024x768.mjpeg"
cPos = 0
startT = 0  
endT = 0
userSum = 0
...
# ✅ replace playsound with pygame sound loader
def play_audio(file):
...
while True:
    ...
    if inFrame == 1:
        if not(isInit):
            play_audio('green.mp3')   # ✅ changed here
            currWindow = im1
            startT = time.time()
            endT = startT
        ...
        if (endT - startT) <= dur:
            ...
            if cPos >= 100:
                ...
                if not(isCinit):
                    ...
                    cEnd = cStart
                    currWindow = im2
                    play_audio('red.wav')   # ✅ changed here
        ...
        frame_resized = cv2.resize(frm, (800, 400))
        window_resized = cv2.resize(currWindow, (800, 400))
        mainWin = np.concatenate((frame_resized, window_resized), axis=0)
